...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " Correlation Features'>";
        options += "<option value='categorical_bar_chart'>Categorical Bar chart</option>";
       
      options += "</optgroup>";
    }
    else {
        options += "<optgroup label='Reduce the number of Correlation Features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
...
function isLimitedCodonPlotAnalysis(analysis) {
  var limited_analyses = new Array("categorical_bar_chart");
