...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " Varionomic Count Features'>";
      ...
      options += "</optgroup>";
        ...
        options += "<optgroup label='Reduce the number of Varionomic Count Features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
         options += "<optgroup label='Available for any number of Varionomic Count Features'>";
        options += "<option value='downsampled_dendogram_plots'>Downsampled Dendogram plot</option>";
        options += "<option value='PCA_3D_label_plots'>Downsampled Hierarchial scatterplot</option>";
