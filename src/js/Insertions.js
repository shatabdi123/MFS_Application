...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " Insertion Count Features'>";
      ...
      options += "</optgroup>";
        ...
        options += "<optgroup label='Reduce the number of Insertion Count Features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
         options += "<optgroup label='Available for any number of Insertion Count Features'>";
