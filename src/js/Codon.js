...
function analysis_change() {
    ...
    else if (analysis_val === "ENC_plot") {
        //Enable Submit for Analysis button, disable Downsampled analysis button
        enable_button("bttn2", "");
        enable_button("bttn3", "");
    }
...
function populate_analysis_menu(count, comp_vals) {
    ...
    options += "<optgroup label='Available for NC and G+C content 3rd position codon compositions only'>"
    options += "<option value='ENC_plot' ";
    if (comp_vals != "Nc,GC3s") { console.log('true; comp_vals = ' + comp_vals);
       options += "title='Select only NC and G+C content 3rd position in the codon compositions menu' disabled";
     }
     options +=">ENC plot</option></optgroup>";
   
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " codon compositions'>";
      ...
      options += "</optgroup>";
        ...
        options += "<optgroup label='Reduce the number of codon compositions to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
         options += "<optgroup label='Available for any number of codon compositions'>";
