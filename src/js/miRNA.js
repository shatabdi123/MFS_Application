...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " miRNA Counts features'>";
        options += "<option value='histogram'>Histogram</option>";
      options += "</optgroup>";
    }
    else {
        options += "<optgroup label='Reduce the number of miRNA Counts features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
...
function isLimitedCodonPlotAnalysis(analysis) {
  var limited_analyses = new Array("histogram");
