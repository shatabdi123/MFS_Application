...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " DNA Methylation Features'>";
      ...
      options += "</optgroup>";
        ...
        options += "<optgroup label='Reduce the number of DNA Methylation Features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
         options += "<optgroup label='Available for any number of DNA Methylation Features'>";
