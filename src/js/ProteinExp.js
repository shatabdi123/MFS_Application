...
function topmenu_change() {
    ...
    analysis_select.innerHTML = populate_analysis_menu(exp_count);
 ...
 */
function populate_analysis_menu(count) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " protein expressions'>";
      ...
      options += "</optgroup>";
      options += "<optgroup label='Available for any number of protein expressions'>";
    }
    else {
        options += "<optgroup label='Reduce the number of protein expression to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
