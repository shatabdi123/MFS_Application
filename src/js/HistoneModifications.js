...
function analysis_change() {
    ...
    if (isLimitedCodonPlotAnalysis(analysis_val)){
        ...
        //label_change(); //More than one label may still be selected, if so the items need to remain disabled
    }
...
function populate_analysis_menu(count, comp_vals) {
    ...
    if (count <= MAX_CODONS_PLOTS) {
      options += "<optgroup label='Available for a max of " + MAX_CODONS_PLOTS + " Histone Modifications features'>";
      ...
      options += "</optgroup>";
        ...
        options += "<optgroup label='Reduce the number of Histone Modifications features to " + MAX_CODONS_PLOTS + " or less to see additional analysis options'>";
    }
    if (label != "no_label") {
         options += "<optgroup label='Available for any number of Histone Modifications features features'>";
